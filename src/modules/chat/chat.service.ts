...
export class ChatService {
  ...
  async getMessengerSidebar(currentUserId: string) {
    // ... code cũ (giữ nguyên)
    const conversations = await this.convRepo.createQueryBuilder('conversation')
    ...
    const result = await Promise.all(conversations.map(async (conv) => {
        ...
        const partner = conv.participants.find(p => p.user_id !== currentUserId);
  ...
  async markAsRead(conversationId: string, currentUserId: string) {
    await this.msgRepo.update(
        { conversation: { id: conversationId }, sender: { user_id: Not(currentUserId) }, is_read: false },
        { is_read: true }
    );
  }
  ...
  async findSupportHandler(studentId: string) {
    ...
    if (latestEnrollment && latestEnrollment.class && latestEnrollment.class.teachers.length > 0) {
    ...
    if (!admin) {
       throw new NotFoundException("Hệ thống chưa có Admin hoặc Giáo viên hỗ trợ");
    }
  ...
  async getUnreadCount(userId: string) {
    return this.msgRepo.createQueryBuilder('message')
